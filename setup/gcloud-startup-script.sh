...
export INSTALL_DIR=/usr/local/opensiddur-app

echo "Setting up the opensiddur-app user..."
useradd -c "app"  app
...
echo "Obtaining opensiddur-app sources..."
mkdir -p /usr/local
cd /usr/local
git clone git://github.com/opensiddur/opensiddur-app.git
cd opensiddur-app
git checkout ${BRANCH}
export SRC=$(pwd)
...
export DNS_NAME="app2-feature.jewishliturgy.org"
export DB_DNS_NAME="db-dev.jewishliturgy.org"
# branch-specific environment settings
if [[ $BRANCH == "master" ]];
then
    export DNS_NAME="app2-prod.jewishliturgy.org"
    export DB_DNS_NAME="db-prod.jewishliturgy.org";
elif [[ $BRANCH == "develop" ]];
then
    export DNS_NAME="app2-dev.jewishliturgy.org"
    export DB_DNS_NAME="db-dev.jewishliturgy.org";
fi
